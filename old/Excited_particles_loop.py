...
laser_x_range=np.linspace(-0.004,0.004,num=50)

fraction_list=[]



for laser_x in tqdm(laser_x_range):
    ...
    fraction = fraction=len(x[excited])/len(x)
...
np.save('cache/fraction_list.npy', fraction_list)
np.save('cache/laser_x_range.npy', laser_x_range)


plt.figure()
plt.title('Fraction of excited particles against laser offset',fontsize=14)
plt.plot(laser_x_range*1e3,fraction_list)
plt.ylabel('Fraction of excited particles(%)',fontsize=12)
plt.xlabel('Horizontal laser offset(mm)',fontsize=12)
